...
static CONTEXT: OnceLock<SGContext> = OnceLock::new();

pub fn initialize(character_data: Vec<u8>, algorithm_data: Vec<u8>) -> Result<&'static SGContext> {
    (*INITIALIZE_CODE)?;
    Ok(CONTEXT.get_or_init(|| SGContext::new(character_data, algorithm_data).unwrap()))
}

pub fn get() -> Option<&'static SGContext> {
    CONTEXT.get()
}
